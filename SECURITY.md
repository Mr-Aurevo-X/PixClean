# Security Policy — PixClean

## Scope (EN)

**PixClean** is a **local Windows** desktop tool (portable `.exe` / local UI).  
...
Image metadata stripping runs **on your machine**; files are not uploaded to Mr-Aurevo-X.

Official builds: only Releases on **https://github.com/Mr-Aurevo-X/PixClean**  
...
Le strip EXIF / GPS / XMP se fait **sur ta machine** ; les fichiers ne sont pas envoyés à Mr-Aurevo-X.
