# Conditions générales d’utilisation — PixClean

Éditeur : Mr-Aurevo-X  
Produit : PixClean  
Copyright (c) 2026 Mr-Aurevo-X. Tous droits réservés.

Les présentes CGU s’appliquent à **PixClean** (app vitrine Mr-Aurevo-X). Le fichier anglais `LICENSE` (PolyForm Noncommercial 1.0.0) est la licence. Ce document ajoute les règles d’usage. Ce n’est pas une seconde licence.
...
Binaires officiels : GitHub Releases `Mr-Aurevo-X/PixClean`.
...
Utilisez l’app conformément à son objet (PixClean). Les actions réseau éventuelles sont décrites dans la politique de confidentialité.
